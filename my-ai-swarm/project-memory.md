...
| 2026-03-23 | #015 | 新增四個統計查詢 API 端點 | 實作每日/每週/每月/每年四個 GET 統計查詢端點，遵循 VSA 架構：每個 Feature 包含 Query、FluentValidation Validator、Handler（AsNoTracking + PaginationResult）、Endpoint（RequireAuthorization + WithTags(Statistics)）。所有端點使用 JWT Bearer 驗證，Handler 透過 UserContext.GetUserIdAsync 確保使用者只能查詢自己的統計資料。建立四個對應的 sealed record Response DTO（DailyStatisticResponse、WeeklyStatisticResponse、MonthlyStatisticResponse、YearlyStatisticResponse）。 | `Features/Statistics/GetDailyStatistics.cs`, `Features/Statistics/GetWeeklyStatistics.cs`, `Features/Statistics/GetMonthlyStatistics.cs`, `Features/Statistics/GetYearlyStatistics.cs`, `Contracts/Statistics/*.cs`, `README.md`, `my-ai-swarm/copilot-service-memory.md` |
