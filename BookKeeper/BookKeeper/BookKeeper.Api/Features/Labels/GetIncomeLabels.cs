...
public static class GetIncomeLabels
{
...
public class GetIncomeLabelsEndpoint : IEndpoint
{
    public void MapEndpoints(IEndpointRouteBuilder app)
    {
        app.MapGet("api/labels/incomes", async (int? page, int? pageSize, ISender sender) =>
        {
            Result<PaginationResult<LabelResponse>> result = await sender.Send(
                new GetIncomeLabels.Query
                {
                    Page = page ?? 1,
